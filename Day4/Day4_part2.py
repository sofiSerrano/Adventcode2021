...
def listabingo():
    ...
    while len(listaBoard)>1:
        ...
        while len(listaBoard)>1 and win==False:
            if searchWinRow(listaBoard,listaBing,cant)!="nada":
                win=True
                winpos=listaBing[cant]
                posBoard=searchWinRow(listaBoard,listaBing,cant)
                listaBoard.pop(posBoard)
